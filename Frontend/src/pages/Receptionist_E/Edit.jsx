...
const Edit = () => {
    ...
    const id = localStorage.getItem('repid')
    ...
    const handelpassword = async (e) =>{
        e.preventDefault();
        try {
            const response = await axios.post('http://localhost:3001/reppass',  {id, password, newpassword});
        ...
        <>
            ...
            <div className='container mx-auto py-6 space-y-5 px-2'>
                ...
                <div className='p-8 space-y-2 border-2 flex flex-col rounded-xl '>
                    ...
                    <input type="tel" value={phonenumber} onChange={(e) => setPhonenumber(e.target.value)} className='p-3  border dark:border-none rounded-lg appearance-none' />
                    ...
                    <div className=' mt-5 flex flex-wrap gap-4 justify-end items-center'>
                        ...
                        <button onClick={()=> navi('/doc/setting')} className='p-3 px-6  bg-[#000000d0] text-white hover:opacity-80  rounded-3xl'>Cancel</button>
                    </div>
